...
interface UpdateCommuneProps {
  setShowModalUpdateCommune: React.Dispatch<React.SetStateAction<boolean>>;
  siteId: number;
  initialData: {
...
const UpdateCommune: React.FC<UpdateCommuneProps> = ({ setShowModalUpdateCommune, siteId, initialData }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      Swal.fire({
        ...
        langueActive?.indice === "en" ? "Municipality updated successfully." : "",
        confirmButtonColor: "#7c3aed",
        background: "#1c2d55",
        color: "#fff",
      }).then(() => {
        setShowModalUpdateCommune(false);
  ...
  return (
    <div className="fixed inset-0 bg-[#111C44] bg-opacity-50 flex items-start justify-center pt-2 z-50">
      ...
      >
        <h2 className="text-xl font-bold mb-4 text-white">{t("updateCommune")}</h2>
        ...
        <button
          onClick={() => setShowModalUpdateCommune(false)}
